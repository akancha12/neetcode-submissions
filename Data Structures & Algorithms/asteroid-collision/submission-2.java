class Solution {
    public int[] asteroidCollision(int[] asteroids) {
        ...
        for (int asteroid : asteroids) {
            if (asteroid >= 0 || currentState.isEmpty()
                || isPositive(currentState.peek()) == isPositive(asteroid)) {
                currentState.push(asteroid);
            } else {
                while (!currentState.isEmpty() && currentState.peek() >= 0
                    && currentState.peek() < Math.abs(asteroid)) {
                    currentState.pop();
                }
